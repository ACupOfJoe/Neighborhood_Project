...
function execute() {
	...
	var timeAutocomplete = new google.maps.places.Autocomplete(document.getElementById('starting_location_string'));
		...
		if (infowindow.marker != marker) {
			...
			$.ajax({
				...
				success: function(response) {
					pageId = Object.keys(response.query.pages)[0];
					infowindow.marker = marker;
					infowindow.setContent('<h3>' + marker.title + '</h3><p>' + durationText + ' away, ' + distanceText + '</p></br>' + '<div>' + response['query']['pages'][String(pageId)]['extract'] + '</div>');
					infowindow.open(map, marker);
					marker.setMap(map);
				}
			});
